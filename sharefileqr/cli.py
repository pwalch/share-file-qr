...
@click.option("--port", type=click.INT, default=4000, help="Port to run the server on.")
@click.option(
    "--browser-display",
    "-b",
    is_flag=True,
...
def cli(file_to_share: str, port: int, browser_display: bool) -> None:
    """
    ...
    if not os.path.isfile(file_to_share):
        raise FileNotFoundError(f"The file you passed does not exist: {file_to_share}")
